...
def salida_segura_act(driver):
	if driver.title != "Oracle Field Service":		
		if driver.find_element_by_xpath('//*[@id="screen-title"]').text == "Agregar actividad":
			driver.find_element_by_xpath('//*[@class="title-flex-item-left"]').click()
			time.sleep(7)
		...
		driver.find_element_by_xpath('//*[@class="user-menu" and @role="button"]').click()
		time.sleep(1)
		driver.find_element_by_xpath('//*[@class="item-caption __logout __logout"]').click()
		time.sleep(5)
...
def salida_Can(driver):
	if driver.title != "Oracle Field Service":
		...
		time.sleep(5)
		WebDriverWait(driver, 90).until(EC.invisibility_of_element_located((By.XPATH,'//*[@class="loading-animated-icon big jbf-init-loading-indicator"]')))
		driver.find_elements_by_xpath('//*[@data-bind="text: initials"]')[1].click()
		time.sleep(1)
		driver.find_element_by_xpath('//*[@class="item-caption __logout"]').click()
		time.sleep(7)
		WebDriverWait(driver, 30).until(EC.invisibility_of_element_located((By.XPATH, '//*[@class="loading-animated-icon big jbf-init-loading-indicator"]')))
	driver.quit()
...
def salida_ot_marcada(driver):
	...
	while x<5:	
		try:
			try:
				botondos = driver.find_element_by_xpath('//*[@id="slot-panel-title" and contains(text(),"Los cambios no se han enviado. ¿Desea guardar un borrador de las actualizaciones?")]')
...
def GetContBotMG(Comando,idbot,idAct):
	if Comando == "Pausar":
		while 1:
...
def GetContBotWF(Comando,idBot,Idactividad):
